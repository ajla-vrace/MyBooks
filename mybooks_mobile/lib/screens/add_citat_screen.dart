...
class _AddCitatScreenState extends State<AddCitatScreen> {
  ...
  try {
    ...
    var stareZnackeIds = znackePrije.result
        .map((x)=>x.znackaId)
        .toSet();
    ...
    await CitatProvider().insert({
      ...
      "brojStranice":
          int.parse(stranicaController.text),
    ...
    if(osvojene.isNotEmpty){

      if(!mounted) return;


      await showMultipleAchievementDialog(
        osvojene,
      );
